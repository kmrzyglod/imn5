...
#ifndef IMN5_IMNMATH_H
#define IMN5_IMNMATH_H
#include <cmath>
#include <iostream>
#include <ostream>
#include <fstream>
...
*/
    static void plot_2d_system(const char* outputname, T** data,int nx,int ny,double dx,double dy){
        ...
        if(plot_params.stype & GNUPLOT_CONTOUR){
            ...
            file << "unset clabel\n";
            if(!(plot_params.stype & GNUPLOT_PM3D)) file << "unset surface\n";
        }
...
template<class T> std::vector<double**> imn<T>::time_data2d;
template<class T> std::vector<double**> imn<T>::time_data1d;
template<class T> imn_gnuplot_params imn<T>::plot_params;


typedef  imn<float>  imnf;
typedef  imn<double>  imnd;
typedef  imn<int>  imni;
#endif //IMN5_IMNMATH_H
